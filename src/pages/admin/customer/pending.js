...
export default function userList() {
    ...
    const listPendingCustomers = async () => {
        try {
            const result = await Api.listPendingCustomers(paginate);
            setCustomers(result)
        } catch (ex) {
    ...
    const approveStore = async (customer_id) => {
        try {
            await Api.approveCustomerStore(customer_id);
            await listPendingCustomers();
        } catch (ex) {
            toast.warning(ex.message)
        }
    }
    ...
    return <>
        <Head>
            <title>Pending Approval B2B Customer List | DoctorOnCall</title>
                        ...
                        <div className="col-lg-12 col-md-12 col-sm-12 col-12">
                            <div className="product-status-wrap">
                                <div className="d-flex align-content-center justify-content-between mb-3">
                                    <h4 className="mb-0">Pending Approval B2B Customer List</h4>
                                </div>
                                <table>
                                    <thead>
                                        <tr>
                                        ...
                                        {customers.results.length > 0 ?
                                            customers.results.map((customer, index) => {
                                                return <tr key={index}>
                                                    ...
                                                    <td>
                                                        <button className="btn btn-success btn-sm" onClick={() => approveStore(customer.id)}>Approve</button>
                                                    </td>
                                                    <td>
