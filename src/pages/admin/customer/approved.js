...
export default function userList() {
    ...
    const listApprovedCustomers = async () => {
        try {
            const result = await Api.listApprovedCustomers(paginate);
            setCustomers(result)
        } catch (ex) {
    ...
    const disapproveStore = async (customer_id) => {
        try {
            await Api.disapproveCustomerStore(customer_id);
            await listApprovedCustomers();
        } catch (ex) {
            toast.warning(ex.message)
        }
    }
    ...
    return <>
        ...
        <Head>
            <title>Approved B2B Customer List | DoctorOnCall</title>
                        ...
                        <div className="col-lg-12 col-md-12 col-sm-12 col-12">
                            <div className="product-status-wrap">
                                <div className="d-flex align-content-center justify-content-between mb-3">
                                    <h4 className="mb-0">Approved B2B Customer List</h4>
                                </div>
                                <table>
                                    <thead>
                                        <tr>
                                        ...
                                        {customers.results.length > 0 ?
                                            customers.results.map((customer, index) => {
                                                return <tr key={index}>
                                                    ...
                                                    <td>
                                                        <button className="btn btn-danger btn-sm" onClick={() => disapproveStore(customer.id)}>Disapprove</button>
                                                    </td>
                                                    <td>
