                ...
                await Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        try {
                            ...
                            await Api.bulkPublishProduct(product_ids);
            ...
            if (incompleteCount != 0) {
                ...
                await Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        try {
                            const product_ids = products.results.filter(({checked})=>checked).map(({id})=>id);
                            await Api.bulkDraftProduct(product_ids);
        ...
        try {
            await Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        const product_ids = products.results.filter(({checked})=>checked).map(({id})=>id);
                        await Api.bulkDeleteProduct(product_ids);
                            ...
                            <div className="product-status-wrap">
                                ...
                                <div className="row mb-3">
                                    ...
                                    <div className="col-md-6 col-12 d-flex align-content-center justify-content-md-end">
                                        ...
                                        &emsp;
                                        <a className="btn btn-primary ms-2 btn-sm" href={basepath(`/admin/product/new`)}>Add Product</a>
                                                ...
                                                return <tr key={index}>
                                                    ...
                                                    <td className="d-flex align-items-start">
                                                        <div className="row">
                                                            <div className="col-12">
                                                                <span className="fw-bold cursor" style={{ wordWrap: 'break-word', wordBreak: 'break-word', whiteSpace: 'normal' }} onClick={() => navigate.push(`/admin/product/${product.id}`)}>{product.data.name}</span>
                                                            </div>
                                                        </div>
                                                    </td>

                                                    <td>
                                                        {product.variants.map((variant, i) => <React.Fragment key={i}>
                                                                <span style={{ wordWrap: 'break-word', wordBreak: 'break-word', whiteSpace: 'normal' }} key={i}>{variant.data.name}</span><br />
                                                            </React.Fragment>
                                                        )}
                                                    </td>
                                                    ...
                                                    <td>
                                                        <button
                                                            ...
                                                            onClick={() => navigate.push(`/admin/product/${product.id}`)}>
                        ...
                        <div className="col-lg-12 col-md-12 col-sm-12 col-12">
                            <div className="px-3 d-flex justify-content-between">
                                ...
                                <div className="text-end" id="foooter">
                                    ...
                                    <button type="button" className="btn btn-sm btn-danger ms-2" onClick={onBulkDelete}>Delete</button>
                                    <button type="button" className="btn btn-sm btn-success ms-2" onClick={onBulkDraft}>Draft</button>
                                    <button type="button" className="btn btn-sm btn-primary ms-2" onClick={onBulkPublish}>Publish</button>
                                </div>
                            </div>
                        </div>
                    </div>
