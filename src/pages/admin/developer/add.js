...
export default function createAdminWebhook() {
    ...
    async function onSubmit() {
        try {
            if (await Api.createAdminWebhook(webhook)) {
                toast.success("Webhook Successfully Create", {
                    autoClose: 1000,
                    onClose: navigate.push("/admin/developer/webhook")
                });
            }
        } catch (ex) {
                                                ...
                                                <div className="mb-3">
                                                    ...
                                                    <div>
                                                        <select defaultValue={webhook.webhook_event} className="form-select" name="webhook_event" onChange={webHookHandler}>
                                                            ...
                                                            <option value="runner_store_report_error">Debug : Store Report Runner</option>
                                                            <option value="runner_refund_report_error">Debug : Refund Report Runner</option>
                                                            <option value="runner_complete_report_error">Debug : Completed Report Runner</option>
                                                            <option value="debug_error">Debug : Error Debug</option>
                                                        </select>
                                                    </div>
                                                </div>
