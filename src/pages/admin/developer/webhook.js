...
export default function Webhook() {
    ...
    const getWebhookList = async () => {
        try {
            const webhook = await Api.getAdminWebhookList(paginate);
            setWebhook(webhook)
        } catch (ex) {
    ...
    const deleteAdminWebhook = async (product_id) => {
        try {
            await Swal.fire({
                title: 'Are you sure?',
                ...
                if (result.isConfirmed) {
                    await Api.deleteAdminWebhook(product_id);
                    Swal.fire(
                        'Deleted!',
                                    ...
                                    <div className="col-lg-10 col-md-10 col-sm-10 col-10">
                                        <div className="breadcomb-wp">
                                            ...
                                            <div className="text-dark">
                                                ...
                                                <span className="mini-click-non"><a href="#" className="">Admin</a> / Developer / Webhook </span>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="product-status-wrap">
                                <div className="d-flex align-content-center justify-content-between mb-3">
                                    ...
                                    <div className="add-product">
                                        <a className="ms-2 mb-2" href={`${process.env.NEXT_PUBLIC_BASEPATH || ""}/admin/developer/add`}>Add Webhook</a>
                                    </div>
                                </div>
                                <table>
                                    <thead>
                                            ...
                                            webhook.results.map((wehbook, index) => {
                                                return <tr key={index}>
                                                    ...
                                                    <td>
                                                        ...
                                                        <button data-toggle="tooltip" title="Trash" className="btn btn-sm btn-danger ms-2" onClick={() => deleteAdminWebhook(wehbook.id)}><i className="fa fa-trash-o" aria-hidden="true"></i></button>
                                                    </td>
                                                </tr>
                                            }) : <></>}
                                    </tbody>
