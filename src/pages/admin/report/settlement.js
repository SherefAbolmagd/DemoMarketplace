...
export default function index() {
    ...
    const getReport = async () => {
        try {
            ...
            const res = await Api.getSettlementReport({ start, end });
            setReport(res);
        } catch (ex) {
    ...
    // Upload Excel File
    const uploadSettlement = async (event) => {
        try {
            ...
            if (await Api.uploadSettlementReport(formData)) {
                toast.success("Settlement Report Uploaded", {
                    autoClose: 1000
                });
                await getReport();
            }
        ...
        <div className="px-5 container-fluid">
                        ...
                        <div className="col-lg-12 col-md-12 col-sm-12 col-12">
                            ...
                            <div className="row mb-3">
                                <div className="col-md-6 col-12 d-flex align-content-center">
                                    <h4 className="mb-3 mb-md-0 fs-6">Settlement Report</h4>
                                </div>

                                <div className="col-md-6 col-12 d-flex align-items-center justify-content-md-end">
                                    <a className="btn btn-primary ms-2 btn-sm"
                                        href={`${process.env.NEXT_PUBLIC_BASEPATH || ""}/api/admin/report/settlement/download?${query}`} >
                                        Download
                                    </a>
                                    
                                    <label htmlFor="addImageSelect">
                                        <a className="btn btn-success btn-sm ms-2">Upload Settlement</a>
                                    </label>
                                    <input type="file" id="addImageSelect" name="ww" className="d-none" accept=".xlsx" onChange={uploadSettlement} multiple={false} />
                                </div>
                            </div>
            ...
            <div className="containter justify-content-start">
                <div className="row">
                    ...
                    <div className="col-sm-12 col-lg-12 col-md-12 col-12">
                        <div className="card mb-3">
                            <div className="p-3 fw-bold"> Daily Settlement List</div>
                            <table className="mx-3 my-3 w-100">
                                ...
                                <tbody>
                                    {
                                        reports.map(({ company_name, company_registration, discount, sub_total, order_count, comission, delivery }, i) =>
                                            <tr>
                                                <td>{i + 1}</td>
                                                <td>{company_name}</td>
                                                ...
                                                <td>RM {Big(delivery || 0).toFixed(2)}</td>
                                                <td>RM {Big(comission || 0).toFixed(2)}</td>
                                                <td>RM {Big(sub_total || 0).sub(discount || 0).sub(comission || 0).sub(delivery || 0).toFixed(2)}</td>
                                            </tr>)
                                    }
